...
export default function Workouts() {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 animate-fade-in">
      {/* Plan Header */}
      <div className="glass-panel p-4 sm:p-6 rounded-[16px] border border-(--border-color-hover) space-y-4">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <div>
            ...
            <h2 className="font-bold text-xl sm:text-2xl text-(--text-primary) capitalize mt-2">{user.plan?.name || "Workout"} Split</h2>
          </div>
          <div className="text-left sm:text-right shrink-0">
            <p className="text-[10px] text-(--text-muted) uppercase font-semibold">Plan Rate</p>
            <p className="text-xl font-bold text-(--text-primary) mt-0.5 tabular-nums">
              ₹{(user.plan?.price || 700).toLocaleString()}{" "}
              <span className="text-xs text-(--text-muted) font-normal">/ {user.plan?.durationDays || 30} days</span>
            </p>
          </div>
        </div>
        <div className="bg-gym-dark/45 border border-(--border-color) p-4 rounded-[6px]">
          <h4 className="text-xs text-(--text-muted) font-bold uppercase tracking-wider mb-1.5">Plan Description</h4>
            ...
            return (
              <div key={item.day} className={`p-3 sm:p-4 rounded-[6px] border transition-all duration-300 ${isToday ? "bg-gym-orange/[0.04] border-gym-orange ring-1 ring-gym-orange/20 shadow-md shadow-gym-orange/5" : "bg-gym-dark/25 hover:bg-gym-dark/40 border-(--border-color) hover:border-(--border-color-hover)"}`}>
                <div className="flex justify-between items-center pb-2.5 border-b border-(--border-color)/40">
                  ...
                  {isToday && <span className="bg-(--text-primary) text-(--bg-canvas) text-[9px] font-bold px-2 py-0.5 rounded-[4px] uppercase tracking-wider">Today</span>}
                </div>
                <div className="mt-3">
                  <h4 className={`font-bold text-xs tracking-wide ${isToday ? "text-gym-orange" : "text-(--text-primary)"}`}>{item.focus}</h4>
