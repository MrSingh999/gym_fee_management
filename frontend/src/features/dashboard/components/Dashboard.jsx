...
import { Users, AlertTriangle, Clock, Calendar, Phone, CheckCircle, RefreshCw, UserPlus, TrendingUp } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      console.error(err);
      setError('Failed to fetch dashboard data. Please make sure the server and database are running.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredMembers = dueMembers.filter((member) => {
    ...
    if (filterType === 'due') {
      return daysDiff >= 0 && daysDiff <= dueTimeframe && member.status !== 'inactive';
    }
    
    // 'all' includes overdue AND upcoming dues within timeframe
    return (daysDiff < 0 || (daysDiff >= 0 && daysDiff <= dueTimeframe)) && member.status !== 'inactive';
  });
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
        <div className="relative">
          ...
          <div className="absolute inset-0 animate-spin rounded-full h-12 w-12 border-2 border-transparent border-b-gym-orange/30" style={{ animationDirection: 'reverse', animationDuration: '1.5s' }}></div>
        </div>
        <p className="text-gym-text-muted font-medium text-sm">Loading fitness metrics...</p>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-lg mx-auto mt-16 animate-fade-in">
        <div className="glass-card p-8 rounded-2xl text-center border-red-500/20">
          ...
          <h3 className="text-lg font-display font-bold text-slate-800 dark:text-white mb-2">Connection Error</h3>
          <p className="text-gym-text-secondary text-sm mb-6 leading-relaxed">{error}</p>
          <button
  ...
  const kpiCards = [
    ...
    {
      label: dueTimeframe === 0 ? 'Due Today' : dueTimeframe === 3 ? 'Due in 3d' : 'Due in 7d',
      ...
      <div className="glass-panel p-4 sm:p-5 md:p-6 rounded-2xl space-y-5 animate-fade-in" style={{ animationDelay: '200ms', animationFillMode: 'both' }}>
        ...
        {filteredMembers.length === 0 ? (
          <div className="text-center py-14 animate-fade-in">
            ...
            <h3 className="text-lg font-display font-bold text-slate-800 dark:text-white mb-1">All Clear</h3>
            <p className="text-gym-text-secondary text-sm max-w-sm mx-auto mb-6">
              No members are currently overdue or have fees due within the selected timeframe.
            </p>
            <button
                ...
                >
                  <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-start gap-4">
                    ...
                    <div className="space-y-2 min-w-0 flex-1">
                      <div>
                        <h4 className="font-bold text-base text-slate-800 dark:text-white truncate">{member.name}</h4>
                        <div className="flex items-center space-x-2 mt-1">
                          <span className={`inline-flex items-center space-x-1 text-[10px] uppercase font-bold tracking-widest px-2 py-0.5 rounded-full ${
                            isOverdue ? 'bg-red-500/15 text-red-400' : 'bg-amber-500/15 text-amber-400'
                          }`}>
                            <span className={`status-dot ${isOverdue ? 'status-dot-overdue' : 'status-dot-due'}`}></span>
                            <span>{member.status}</span>
                          </span>
                          <span className="text-xs text-gym-text-muted capitalize">{member.membershipType}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center text-xs text-gym-text-secondary space-x-1.5">
                        ...
                        <span>
                          Due: <strong className="text-slate-800 dark:text-white">{formatDate(member.endDate)}</strong>
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex sm:flex-col justify-between items-end sm:items-stretch sm:min-h-[90px] border-t border-gym-border/30 sm:border-none pt-3 sm:pt-0 mt-1 sm:mt-0 ml-0 sm:ml-4 shrink-0">
                      <div className="text-left sm:text-right">
                        <p className="text-[11px] text-gym-text-muted uppercase font-semibold">Amount</p>
                        <p className="text-lg font-bold text-slate-800 dark:text-white mt-0.5 tabular-nums">₹{member.feeAmount.toLocaleString()}</p>
                      </div>

                      <div className="mt-0 sm:mt-3 text-right">
                        {isOverdue ? (
                          <p className="text-[11px] text-red-400 font-semibold mb-1.5 sm:mb-2">Overdue by {Math.abs(daysDiff)}d</p>
                        ) : (
                          <p className="text-[11px] text-amber-400 font-semibold mb-1.5 sm:mb-2">Expires in {daysDiff}d</p>
                        )}
